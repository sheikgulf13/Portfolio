...
const ProjectCard = ({ title, desc, imgUrl, github, liveSite }) => {
  return (
    <motion.div layout className='glass relative [background:linear-gradient(135deg,rgba(255,255,255,0.5),rgba(255,255,255,0.3))] dark:[background:linear-gradient(135deg,rgba(255,255,255,0.2),rgba(255,255,255,0))] !transition-colors !duration-1000 dark:!transition-all dark:!duration-500 project-card w-[280px] h-[40vh] md:h-[60vh] p-5 flex flex-col hover:cursor-pointer social-link-3 dark:social-link-3'>
        <Image src={imgUrl} alt='project img' className='rounded-lg w-[220px] md:w-[260px] h-[120px] md:h-[150px] mb-5' unoptimized />
        <h4 className='text-xl break-words mb-1 text-black dark:text-white font-bold'>{title}</h4>
        <p className='!break-words text-black dark:text-white font-light text-sm font-["Monserrat"]'>{desc}</p>
